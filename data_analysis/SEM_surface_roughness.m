...
gauss_fit_residue_tot = fit(x_gauss_data_residue_tot, transpose(y_gauss_data_residue_tot), 'gauss1');
...
m = 3;
figure(1)
% sgtitle(strcat(material, ", ", disksize, " disk"))
subplot(n,m,1)
imshow(X)
title('Original image')
subplot(n,m,3)
imshow(filtered_disk)
title(strcat('Filtered image'))
subplot(n,m,2)
imshow(disk_blackwhite)
title(strcat("Black and whited image"))
subplot(n,m,4)
imshow(contour)
title(strcat('Largest contour'))
subplot(n,m,6)
plot(circumference_len,dr)
xlabel('Surface length (\mum)')
ylabel('\deltar (nm)')
title('Surface roughness + waviness')
subplot(n,m,5)
plot(r_x_c.*scaling, r_y_c.*scaling)
hold on
plot(r_coord(sorted_theta_vec_indices,2).*scaling,r_coord(sorted_theta_vec_indices,1).*scaling)
xlabel('x (\mum)')
ylabel('y (\mum)')
legend('Fitted circle to surface',"SEM'ed surface")
title('Disk surface + a fitted circle')
axis equal
subplot(n,m,7)

plot(circumference_len, dr_acf)
hold on
plot(circumference_len, y_dat_fitted)
title('Autocorrelation of \deltar')
xlabel('Surface length (\mum)')
ylabel('Autocorrelation (nm^2)')
legend(strcat("L_c = ", num2str(abs(gauss_coeffs_total(3)*10^3)), "nm\newline", "\sigma = ",num2str(sqrt(gauss_coeffs_total(1))), "nm"))



subplot(n,m,8)
plot(circumference_len, residue)
xlabel('Surface length (\mum)')
ylabel('\deltar (nm)')
title('Surface roughness only')

subplot(n,m,9)

plot(circumference_len, residue_acf)
hold on
plot(gauss_fit_residue)
title('Autocorrelation of surface roughness only')
xlabel('Surface length (\mum)')
ylabel('Autocorrelation (nm^2)')
